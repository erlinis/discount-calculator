...
function App() {
   ...
   */
  function onAddDiscount(price, amount, discount, salePrice, saving, description) {
    var discountRow = {
      id: Date.now(),
      price: Number(price),
    ...
    <>
      <header className="banner">
        <picture className="logo">
          ...
          <img src="images/logo/ticket-logo-1x.png" alt="logo" width="50" height="32" />
        </picture>
        <h1 className="text-2xl font-bold">Discount Calculator</h1>
      </header>
      <div className="container-wrapper">
        <div className="">
          <Calculator priceInputRef={priceInputRef} onAddDiscount={onAddDiscount} />
          <DiscountList discounts={state.discounts} onDeleteDiscount={onDeleteDiscount} />
        </div>
        {state.showInstallMessage ? <AddHomePopup onPress={onPopupPress} /> : null}
      </div>
    </>
  )
}
